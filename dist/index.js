...
__export(src_exports, {
  default: () => AquaProtocol
});
module.exports = __toCommonJS(src_exports);

// src/utils/utils.ts
var import_crypto = require("crypto");
var import_sha2 = require("multihashes-sync/sha2");
...
function prepareNonce() {
  const seed = (0, import_crypto.randomBytes)(32);
  return Buffer.from(seed).toString("base64url");
}
function getHashSum(content) {
  const encoder = new TextEncoder();
  const contentBytes = encoder.encode(content);
  const hash = import_multiformats.bytes.toHex(import_sha2.sha256.digest(contentBytes).bytes);
  return hash;
}
...
function sha256Hasher(data) {
  let result = import_crypto2.default.createHash("sha256").update(data).digest("hex");
  return result;
}

// src/core/revision.ts
var import_merkletreejs2 = require("merkletreejs");
...
async function verifySignature(signature) {
  ...
  const tree = new import_merkletreejs.default(cleanedLeaves, sha256Hasher, {
    duplicateOdd: false
  });
...
var verifyEth = async (ethNetwork, transactionHash, expectedMR, expectedTimestamp) => {
  const provider = import_ethers2.ethers.getDefaultProvider(ethNetwork);
...
function generateGenesisRevisionUtil(file_name, file_data) {
  ...
  const tree = new import_merkletreejs2.MerkleTree(cleanedLeaves, sha256Hasher, {
    duplicateOdd: false
  });
